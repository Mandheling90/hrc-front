pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = 'hmp-cc-frontend'

        // GitLab 저장소 (사설망)
        GITLAB_HOST = '10.101.51.7'
        GITLAB_REPO = "http://${GITLAB_HOST}/kumc/frontend.git"
                ...
                container('git') {
                    ...
                    withCredentials([usernamePassword(
                        ...
                        sh '''
                            ...
                            sed -i '/name: FRONTEND_IMAGE_PLACEHOLDER/{n;n;s|^[[:space:]]*newTag:.*|    newTag: "'"${IMAGE_TAG}"'"|;}' overlays/production/kustomization.yaml
                            ...
                            # 변경사항 커밋 및 푸시
                            git add .
                            git diff --staged --quiet || git commit -m "Update frontend image to ${IMAGE_TAG}"
                            git push
