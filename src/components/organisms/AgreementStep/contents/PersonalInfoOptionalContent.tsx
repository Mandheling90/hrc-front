...
export const PersonalInfoOptionalContent: React.FC = () => {
  ...
  if (isAnam || isGuro || isAnsan) {
    return <ReferralCenterPersonalInfoOptionalContent hospitalName={hospital.name.hospital} />
  }
...
const ReferralCenterPersonalInfoOptionalContent: React.FC<{ hospitalName: string }> = ({ hospitalName }) => {
  return (
    <div className={styles.privacySection}>
      ...
      <div className={styles.privacySectionItem}>
        <p className={styles.privacySectionTitle}>2. 수집하려는 개인정보의 항목</p>
        <div className={styles.privacySectionContent}>
          <p>- 선택 항목 : 병원홈페이지 주소, 출신학교, 진료과, 세부전공</p>
        </div>
      </div>
