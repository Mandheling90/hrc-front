...
import type { BasicTreatmentStepData, StepRef } from '@/types/partner-application'
...
export interface BasicTreatmentStepProps {
  ...
  totalSteps?: number
  /** 초기값 (임시저장 불러오기용) */
  defaultValues?: Partial<BasicTreatmentStepData>
}
...
export const BasicTreatmentStep = forwardRef<StepRef<BasicTreatmentStepData>, BasicTreatmentStepProps>(
  ({ currentStep = 7, totalSteps = 8, defaultValues }, ref) => {
    // 관리 항목 상태
    const [management, setManagement] = useState<Record<string, boolean>>(
      defaultValues?.management ?? DEFAULT_MANAGEMENT
    )

    // 처방 항목 상태
    const [prescription, setPrescription] = useState<Record<string, boolean>>(
      defaultValues?.prescription ?? DEFAULT_PRESCRIPTION
    )

    // 드레싱 항목 상태
    const [dressing, setDressing] = useState<Record<string, boolean>>(defaultValues?.dressing ?? DEFAULT_DRESSING)

    // 처치 항목 상태
    const [treatment, setTreatment] = useState<Record<string, boolean>>(defaultValues?.treatment ?? DEFAULT_TREATMENT)

    // 기타 항목 상태
    const [otherItems, setOtherItems] = useState<string[]>(defaultValues?.otherItems ?? ['', ''])
...
BasicTreatmentStep.displayName = 'BasicTreatmentStep'
