...
import type { CareSystemStepData, StepRef } from '@/types/partner-application'
...
export interface CareSystemStepProps {
  ...
  totalSteps?: number
  /** 초기값 (임시저장 불러오기용) */
  defaultValues?: Partial<CareSystemStepData>
}

export const CareSystemStep = forwardRef<StepRef<CareSystemStepData>, CareSystemStepProps>(
  ({ currentStep = 5, totalSteps = 8, defaultValues }, ref) => {
    // 간병 시스템 상태
    const [integratedNursingCare, setIntegratedNursingCare] = useState(defaultValues?.integratedNursingCare ?? '유')
    const [guardianNursingCare, setGuardianNursingCare] = useState(defaultValues?.guardianNursingCare ?? '유')
    ...
    const [isolationTypeOther, setIsolationTypeOther] = useState(defaultValues?.isolationTypeOther ?? '')
...
CareSystemStep.displayName = 'CareSystemStep'
