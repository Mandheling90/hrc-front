...
import { mapClinicStepsToApiInput, type ClinicAllStepData } from '@/utils/partnerApplicationMapper'
...
export default function ClinicApplicationPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        console.error('[협력의원 접근체크] EHR 조회 실패:', err)
      }
    })()
  ...
  const handleNext = async () => {
    ...
    if (currentStep < totalSteps) {
      ...
      try {
        ...
        clearDraftCookie('clinic', hospital.id)
        setIsComplete(true)
      } catch (error: unknown) {
  ...
  const handleSaveDraft = () => {
    const allData = collectAllData()
    saveDraftToCookie('clinic', hospital.id, allData as unknown as Record<string, unknown>)
    setAlertModal({ isOpen: true, message: '임시저장이 완료되었습니다.' })
  }

  // 임시저장 불러오기 핸들러 (쿠키에서 불러오기)
  const handleLoadButtonClick = () => {
    const loaded = loadDraftFromCookie('clinic', hospital.id) as ClinicAllStepData | null
    if (loaded) {
          ...
          <div className={styles.content}>
            {isComplete ? (
              ...
              <CompleteStep
                userId={user?.userId ?? ''}
                email={user?.email ?? ''}
                title={`${hospital.name.full} 협력의원 신청이 완료되었습니다.`}
