...
import type { HospitalConfig, HospitalId } from '@/types/hospital'

const VALID_HOSPITALS: HospitalId[] = ['anam', 'guro', 'ansan']

// 요청 hostname → 병원 ID. middleware.ts와 동일 규칙.
function hospitalIdFromHost(host: string | null | undefined): HospitalId | null {
  if (!host) return null
...
async function resolveHospital(): Promise<{ hospitalId: HospitalId; hospital: HospitalConfig }> {
  const h = await headers()
  const hospitalId = hospitalIdFromHost(h.get('host')) ?? getCurrentHospitalId()
  return { hospitalId, hospital: getHospitalConfig(hospitalId) }
}

function buildSiteContext(hospitalId: HospitalId, hospital: HospitalConfig) {
  ...
  return { SITE_URL, OG_IMAGE_PATH, SITE_DESCRIPTION }
}

export async function generateMetadata(): Promise<Metadata> {
  const { hospitalId, hospital } = await resolveHospital()
  const { SITE_URL, OG_IMAGE_PATH, SITE_DESCRIPTION } = buildSiteContext(hospitalId, hospital)
...
export default async function RootLayout({
  ...
  const { hospitalId, hospital } = await resolveHospital()
  const { SITE_URL, OG_IMAGE_PATH } = buildSiteContext(hospitalId, hospital)
